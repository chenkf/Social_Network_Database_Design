...
int Bnode_inner::insert(Bnode* child, int idx) { //adds children to inner_node children array (bnode* array)
    assert(num_children < BTREE_FANOUT); // node is full
    for (int i = num_children; i > idx; --i) //5 children, inserting at index 2
        children[i] = children[i-1]; //children[5]=children[4] , children[4]=children[3], children[3]=children[2] //shifting children one over to make room for child insertion at index idx  
    children[idx] = child;
    num_children++;
    return idx;
}
