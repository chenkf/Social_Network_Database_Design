...
WHERE u.CURRENT_CITY IS NOT NULL AND u.CURRENT_STATE IS NOT NULL AND u.CURRENT_COUNTRY IS NOT NULL;
...
WHERE PHOTO_CREATED_TIME IS NOT NULL 
...
WHERE t1.ALBUM_VISIBILITY in ('EVERYONE', 'FRIENDS', 'FRIENDS_OF_FRIENDS', 'MYSELF', 'CUSTOM')
AND t1.ALBUM_NAME IS NOT NULL
AND t1.ALBUM_CREATED_TIME IS NOT NULL
AND t1.ALBUM_LINK IS NOT NULL 
AND t1.ALBUM_VISIBILITY IS NOT NULL;
...
AND t1.TAG_CREATED_TIME IS NOT NULL
AND t1.TAG_X_COORDINATE IS NOT NULL 
AND t1.TAG_Y_COORDINATE IS NOT NULL;
...
WHERE t1.EVENT_CITY = t2.CITY_NAME
	...
	AND t1.EVENT_START_TIME IS NOT NULL
	AND t2.CITY_ID IS NOT NULL
	AND t1.EVENT_END_TIME IS NOT NULL;
